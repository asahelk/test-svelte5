...
<script>
	...
	function refresh() {
		...
		if (scrollDirection === DIRECTION.VERTICAL) {
			...
			innerStyle = `grid-template-rows: repeat(${itemCount},1fr);height:${totalSize}px;`;
		} else {
			wrapperStyle = `height:${height}${heightUnit};width:${width}${widthUnit};`;
			// innerStyle = `min-height:100%;width:${totalSize}px;`;
			innerStyle = `grid-template-columns: repeat(${itemCount},1fr);width:${totalSize}px;`;
		}
	...
	function getStyle(index, sticky) {
		...
		if (scrollDirection === DIRECTION.VERTICAL) {
			...
			if (sticky) {
				...
				style += `position:sticky;grow:0;z-index:99;top:0; grid-row: ${index + 1}`;
			} else {
				// style += `position:absolute;top:${offset}px;`;
				// style += `--child: ${index + 1}; grid-row: var(--child);`;
			...
			if (sticky) {
				...
				style += `position:sticky;grow:0;z-index:99;left:0; grid-column: ${index + 1}`;
			} else {
